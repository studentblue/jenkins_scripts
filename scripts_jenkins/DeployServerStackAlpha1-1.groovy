					...
					cloud:
					{
						...
						[
							...
							},
							{
								...
								"template": "{{network}}",
								"watch":{ "network": "root.Node.networks.name"}
		...
		if( networkArray )
		{
			networkArray.each
			{
				network ->
					...
					def networkKey2 = networks[0].Name
					
					outNetworks.put( networkKey2 , [:] )
					
					outNetworks[networkKey2].put( "name", networks[0].Name)
					outNetworks[networkKey2].put( "subnet", networks[0].IPAM.Config[0].Subnet)
					outNetworks[networkKey2].put( "gateway", networks[0].IPAM.Config[0].Gateway)

					
					outNetworks[networkKey2].put( "containers", [])
					...
					{						
						container.each
						{
							key, value ->
								...
								outNetworks[networkKey2]["containers"].add(
									[
...
def generateNodeStart(nodes)
{
	
	def simpleNetworkList = generateSimpleNetworkList( nodes[0] )
	def simpleContainerList = generateSimpleContainerList(nodes[0])
	...
	if( nodes[0].networks )
	{
		def keys = nodes[0].networks.keySet()
		...
		node.put( "networks", 
			[
				"name": nodes[0].networks[keys[0]].name, "subnet": nodes[0].networks[keys[0]].subnet, "gateway": nodes[0].networks[keys[0]].gateway,
				"removeNetwork": false
			]
		)
		
		if( nodes[0].networks[keys[0]].containers )
		{
			node.networks.put("containers", [:] )
			nodes[0].networks[keys[0]].containers.each
			{
				container ->
					
					node.networks.containers.put("${container.name}",
					[
						"name": container.name,
						"mac": container.mac,
						"ip4": container.ip4,
...
def generateSimpleNetworkList(node)
{
	def list = []
	
	if( node.networks )
	{
		return node.networks.keySet()
	}
	
	return list
}

def generateSimpleContainerList(node)
{
	def list = []
	
	if( node.networks )
	{
		def keys = node.networks.keySet()
		
		for( key in keys )
		{
			for ( container in node.networks[key].containers )
			...
			tags.each
			{
				tag ->
					...
					if( tag.name.contains( SR_TAG_ID ) )
						srBuildImages.add( image + ":" + tag.name )
					
					if( tag.name.contains( AUTH_TAG_ID ) )
						authBuildImages.add( image + ":" + tag.name )
					
					if( tag.name.contains( GATEWAY_TAG_ID ) )
						gatewayBuildImages.add( image + ":" + tag.name )
					
					if( tag.name.contains( EH_TAG_ID ) )
						ehBuildImages.add( image + ":" + tag.name )
					
					if( tag.name.contains( GK_TAG_ID ) )
						gkBuildImages.add( image + ":" + tag.name )
					
					if( tag.name.contains( ORCH_TAG_ID ) )
						orchBuildImages.add( image + ":" + tag.name )
...
jsonEditorOptions.schema.properties.Images.items.oneOf[1].properties.deployImage.enum = srBuildImages

jsonEditorOptions.schema.properties.Images.items.oneOf[2].properties.deployImage.enum = authBuildImages

jsonEditorOptions.schema.properties.Images.items.oneOf[3].properties.deployImage.enum = gatewayBuildImages

jsonEditorOptions.schema.properties.Images.items.oneOf[4].properties.deployImage.enum = ehBuildImages

jsonEditorOptions.schema.properties.Images.items.oneOf[5].properties.deployImage.enum = gkBuildImages

jsonEditorOptions.schema.properties.Images.items.oneOf[6].properties.deployImage.enum = orchBuildImages
...
if( nodes )
{
	...
	nodes.each
	{
		node ->
			...
			def simpleNetworkList = []
			def simpleContainerList = []
				...
				key, network ->
						...
						[
							...
							[
								...
								"gateway": ["type": "string", "default": network.gateway, "readOnly": true],
								"removeNetwork": ["type": "boolean", "default": false, "format": "checkbox"]
							]
						]
						
						//~ if( !startNetwork)
							//~ startNetwork = ["name": network.name,"subnet": network.subnet, "gateway": network.gateway, "removeNetwork": false]
						
						if( network.containers )
						{
							net1.properties.put( "containers",
							[
								"type": "object", "format": "categories", "properties": [:], "additionalProperties": false,
								"required": []
							])
							
							net1.put( "additionalProperties", false)
							net1.put( "required", ["containers"])
							
							network.containers.each
							{
								container ->
									
									def c1 = 
									[
										"type": "object", "title": container.name,
										"properties":
										[
											"name": ["type": "string", "default": container.name, "readOnly": true],
											"mac": ["type": "string", "default": container.mac, "readOnly": true],
											"ip4": ["type": "string", "default": container.ip4, "readOnly": true],
											"image": ["type": "string", "default": container.image, "readOnly": true],
											"status":  ["type": "string", "default": container.status, "readOnly": true],
											"running": ["type": "boolean", "default": container.running, "format": "checkbox", "readOnly": true],
											"error": ["type": "string", "default": container.error, "readOnly": true],
											"removeContainer": ["type": "boolean", "default": false, "format": "checkbox"]
											
										],
										"required": ["name", "mac", "ip4", "image"],
										"additionalProperties": false
									]
									...
									//~ startContainers.put(
										//~ "${container.name}",
										//~ [
											//~ "name": container.name, "mac": container.mac, "ip4": container.ip4,
											//~ "image": container.image, "status": container.status, "running": container.running,
											//~ "error": container.error, "removeContainer": false
											
										//~ ]
									//~ )
									//~ net1.properties.containers.required.add(container.name)
									net1.properties.containers.properties.put("${container.name}", c1 )
									
									simpleContainerList.add(container.name)
							}
							
							//~ if( ! startNetwork.containers )
							//~ {
								//~ startNetwork.put("containers", startContainers)
							//~ }
								
						}
						
						simpleNetworkList.add(network.name)
						
						nodeProperties.properties.networks.oneOf.add(net1)
			}
			
			
			if( ! nodeProperties.properties.networks.oneOf )
			{
				//~ nodeProperties.properties.networks = ["type": "string", "default": "No Networks Found", "readOnly": true]
				nodeProperties.properties.networks = ["type": "boolean", "default": false, "readOnly": true, "options": ["hidden": true]]
			...
			else
			{
				...
				//~ {
					...
					//~ [
						//~ "name": nodes[0].name, "kernel": nodes[0].kernel, "noNode": false,  "os": nodes[0].os ,
						//~ "networks": startNetwork, "simpleNetworkList": simpleNetworkList, "simpleContainerList": simpleContainerList
					//~ ]
				...
				nodeProperties.properties.put("simpleNetworkList", [ "type": "array", "items": ["type": "string"], "default": simpleNetworkList , "options":
				["hidden": true]] )
			
			
				nodeProperties.properties.put("simpleContainerList", [ "type": "array", "items": ["type": "string"], "default": simpleContainerList , "options":
				["hidden": true]] )
...
else
{
	...
	jsonEditorOptions.startval.put("Node", [ "name": "No Node Online", "noNode": true ] )
